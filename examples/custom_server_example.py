...
async def main():
    ...
    custom_config = {
        ...
        "env": {},
        "upload_files": {
            "/tmp/custom_mcp_server.py": CUSTOM_SERVER_CODE
        }
    }
    ...
    try:
        ...
        try:
            # Test addition
            add_result = await runner.execute_tool("custom", "add", {
                "a": 15,
                "b": 27
            })
            ...
            # Test multiplication
            mult_result = await runner.execute_tool("custom", "multiply", {
                "a": 8,
                "b": 9
            })
            ...
            # Test power
            power_result = await runner.execute_tool("custom", "power", {
                "base": 2,
                "exponent": 10
            })
        ...
        sample_text = "Hello, World! This is a sample text for analysis. It contains multiple sentences and words."
        
        try:
            # Get text statistics
            stats_result = await runner.execute_tool("custom", "text_stats", {
                "text": sample_text
            })
            ...
            for transform in transforms:
                transform_result = await runner.execute_tool("custom", "transform_text", {
                    "text": short_text,
                    "operation": transform
                })
        ...
        try:
            ...
            values = []
            for i in range(3):
                result = await runner.execute_tool("custom", "power", {
                    "base": i + 2,
                    "exponent": 2
                })
            ...
            total = values[0]
            for val in values[1:]:
                total = await runner.execute_tool("custom", "add", {
                    "a": total,
                    "b": val
                })
            ...
            report_text = f"Mathematical Analysis Report: The sum of squares from 2^2 to 4^2 equals {total}. This demonstrates the power of custom MCP servers."
            
            report_stats = await runner.execute_tool("custom", "text_stats", {
                "text": report_text
            })
